...
def compare_parts_list(dxf_labels_file, circuit_symbols_file):
    ...
    try:
        ...
        output.append(f"- 図面に不足しているラベル総数: {len(missing_in_dxf_expanded)}")
        output.append(f"- 回路記号に不足しているラベル総数: {len(missing_in_circuit_expanded)}")
        output.append("")
